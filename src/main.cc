...
int main(){
    for(unsigned int b=4; b<=20; b+=2){
        ...
        string hitprofilID = "prof";
        string hitprofilTitle = "Muon hit profil in RPC part. B";
        string rpcprofilID = "rpc";
        string rpcprofilTitle = "Muon hit profil in RPC";
        string distrpcID = "rpc-distance";
        string distrpcTitle = "Path of triggered muons through rpc";
        string muondistribID = "muons";
        string muondistribTitle = "Triggered muon distribution in RPC plane";
        string muonoriginID = "origin";
        ...
        string distscintID = "scint-distance";
        string distscintTitle = "Path of triggered muons through scintillators";
        ...
        string hitprofilIDF = "prof_f";
        string hitprofilTitleF = "Forward muon hit profil in RPC partition B";
        string rpcprofilIDF = "rpc_f";
        string rpcprofilTitleF = "Forward muon hit profil in RPC";
        string distrpcIDF = "rpc-distance_f";
        string distrpcTitleF = "Path of triggered forward muons through rpc";
        string muondistribIDF = "muons_f";
        string muondistribTitleF = "Triggered forward muon distribution in RPC plane";
        string muonoriginIDF = "origin_f";
        string muonoriginTitleF = "Triggered forward muon origin on the generation plane";
        string muondirIDF = "direction_f";
        string muondirTitleF = "Triggered forward muon direction components";
        string distscintIDF = "scint-distance_f-";
        string distscintTitleF = "Path of triggered forward muons through scintillators";
        ...
        string hitprofilIDB = "prof_b";
        string hitprofilTitleB = "Backward muon hit profil in RPC B part.";
        string rpcprofilIDB = "rpc_b";
        string rpcprofilTitleB = "Backward Muon hit profil in RPC";
        string distrpcIDB = "rpc-distance_b";
        string distrpcTitleB = "Path of triggered backward muons through rpc";
        string muondistribIDB = "muons_b";
        string muondistribTitleB = "Triggered backward muon distribution in RPC plane";
        string muonoriginIDB = "origin_b";
        string muonoriginTitleB = "Triggered backward muon origin on the generation plane";
        string muondirIDB = "direction_b";
        string muondirTitleB = "Triggered backward muon direction components";
        string distscintIDB = "scint-distance_b";
        string distscintTitleB = "Path of triggered backward muons through scintillators";
                        ...
                        if(isInTrapezoid(RPCPos,Rpc.part[p].corners)){
                            for(int s = 0; s < 32; s++){
                                if(isInTrapezoid(RPCPos,Rpc.part[p].strips[s])){
                                    ...
                                    if(p == 1) HitProfil->Fill(s+1);

                                    if(direction.second > PI){
                                        RPCProfilF->Fill(p,s+1);
                                        if(p == 1)  HitProfilF->Fill(s+1);
                                    } else {
                                        RPCProfilB->Fill(p,s+1);
                                        if(p == 1) HitProfilB->Fill(s+1);
                                    }
                                }
                            }
                        }
