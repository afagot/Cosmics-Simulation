...
const Vector V_rpc = {1/sin(alpha),0.,0.};
const Vector U_rpc = {cos(alpha),0.,sin(alpha)};
const Vector W_sct = {1.,0.,0.};
const Vector V_sct = {0.,-cos(beta),sin(beta)};
const Vector U_sct = {0.,sin(beta),cos(beta)};
...
Point getHitPositionBigFaces(const Point& P, const Direction& D, double offset){
	Point I;
...
Point getHitPositionMediumFaces(const Point& P, const Direction& D, double offset){
	Point I;
...
Point getHitPositionSmallFaces(const Point& P, const Direction& D, double offset){
	Point I;
...
void setScint(Scintillator& scint, const Point& P, const Direction& D, double offset){
	scint.MuonHit[0] = getHitPositionBigFaces(P,D,offset);
	scint.MuonHit[1] = getHitPositionBigFaces(P,D,offset+TRIGGER_THICK);
	scint.MuonHit[2] = getHitPositionSmallFaces(P,D,0.);
	scint.MuonHit[3] = getHitPositionSmallFaces(P,D,TRIGGER_LENGTH);
	scint.MuonHit[4] = getHitPositionMediumFaces(P,D,0.);
	scint.MuonHit[5] = getHitPositionMediumFaces(P,D,TRIGGER_WIDTH);
}
...
bool isInBigFace(const Point& I, double offset){
...
bool isInMediumFace(const Point& I, double Yoffset, double Zoffset){
	bool x_condition = (I.x >= a_X0 && I.x <= (a_X0+TRIGGER_LENGTH));
...
bool isInSmallFace(const Point& I, double offset){
...
bool isInBottomScintillator(const Scintillator& scint){
	return isInBigFace(scint.MuonHit[1],TRIGGER_THICK);
}

bool isInTopScintillator(const Scintillator& scint){
	return isInBigFace(scint.MuonHit[0],TRIGGER_OFFSET);
}
...
//Get the distance traveled through the scintillators
double getDistanceThroughBottomScint(const Scintillator& scint){
	double d = 0.;
	unsigned int i = 0;

	if(isInBigFace(scint.MuonHit[0],0.))
		i = 0;
	else if(isInSmallFace(scint.MuonHit[2],0.))
		i = 2;
	else if(isInSmallFace(scint.MuonHit[3],0.))
		i = 3;
	else if(isInMediumFace(scint.MuonHit[4],0.,0.))
		i = 4;
	else if(isInMediumFace(scint.MuonHit[5],0.,TRIGGER_WIDTH))
		i = 5;
...
double getDistanceThroughTopScint(const Scintillator& scint){
	double d = 0.;
	unsigned int i = 0;

	if(isInBigFace(scint.MuonHit[1],TRIGGER_OFFSET+TRIGGER_THICK))
		i = 1;
	else if(isInSmallFace(scint.MuonHit[2],TRIGGER_OFFSET))
		i = 2;
	else if(isInSmallFace(scint.MuonHit[3],TRIGGER_OFFSET))
		i = 3;
	else if(isInMediumFace(scint.MuonHit[4],TRIGGER_OFFSET,0.))
		i = 4;
	else if(isInMediumFace(scint.MuonHit[5],TRIGGER_OFFSET,TRIGGER_WIDTH))
		i = 5;
